...
# --- Проверка статуса товара на модерации --- 
@controller.get('/get_all_product_moder_status')
async def get_all_product_moder_status():
    res = await Product.get_or_none()
    return res.moderation if res and res.moderation else False


# --- Проверка статуса товара на модерации определённого продавца --- 
@controller.get('/get_product_moder_status')
async def get_product_moder_status(company_name: int):
    res = await Product.get_or_none(company_name=company_name)
    return res.moderation if res and res.moderation else False
